...
module.exports = {
  ...
  addThemeToJson: function (env, theme, custom) {
    return new Promise((resolve, reject) => {
      ...
      var obj = {themes: _.uniq(_.concat(env.settings.themes, [name]))}
      updateJson(file, obj, (error) => {
        if (error) {
          throw error
          reject('Impossible to write in JSON file.')
        }
          ...
          if (custom) {
            this.ignoreExtension(env, theme, 'theme')
          }
  ...
  addPluginToJson: function (env, plugin, custom) {
    return new Promise((resolve, reject) => {
      ...
      var obj = {plugins: _.uniq(_.concat(env.settings.plugins, [name]))}
      updateJson(file, obj, (error) => {
        if (error) {
          throw error
          reject('Impossible to write in JSON file.')
          ...
          if (custom) {
            this.ignoreExtension(env, plugin, 'plugin')
          }
