...
module.exports = {
  ...
  addProjectNameToJson: function (env, name) {
    return new Promise((resolve, reject) => {
      var file = path.resolve(`${env.cwd}/wpleasefile.json`)
      var obj = {name: name}
   ...
   * @param name The project name to add in the file
   */
  addCustomThemeToJson: function (env, theme) {
    return new Promise((resolve, reject) => {
      var file = path.resolve(`${env.cwd}/wpleasefile.json`)
      var obj = {themes: _.union(env.settings.themes, [`@${theme}`])}
      updateJson(file, obj, function (error) {
        if (error) {
          throw error
          reject('Impossible to write in JSON file.')
          ...
          console.log('Success:', `add custom theme "${theme}" to the wpleasefile.json`)
          console.log('')
          resolve(true)
        }
      })
  ...
  /**
   * Add the custom plugin to the wpleasefile.json
   * @param plugin The plugin name to add in the file
   */
  addCustomPluginToJson: function (env, plugin) {
    return new Promise((resolve, reject) => {
      var file = path.resolve(`${env.cwd}/wpleasefile.json`)
      var obj = {plugins: _.union(env.settings.plugins, [`@${plugin}`])}
      updateJson(file, obj, function (error) {
        if (error) {
          throw error
          reject('Impossible to write in JSON file.')
          ...
          console.log('Success:', `add custom plugin "${plugin}" to the wpleasefile.json`)
          console.log('')
          resolve(true)
        }
      })
